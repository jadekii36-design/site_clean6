...
def _validate_file(f, allowed_ext, max_mb, label="File"):
    ...
    if ext == ".heic":
        raise forms.ValidationError("iPhone HEIC not supported. Please convert to JPG/PNG/WebP before upload.")

    if ext not in allowed_ext:
        raise forms.ValidationError(f"{label} type invalid: {ext}")
    ...
    if size_mb > max_mb:
        raise forms.ValidationError(f"{label} too large ({size_mb:.1f}MB). Max {max_mb}MB.")
